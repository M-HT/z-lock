	...
	switch(TskBuf[id].step){
		case	0:	// init
			...
			if(TskBuf[id].tskid & TSKID_POSSET){
				...
				TskBuf[id].py = (Rand() % 128) - (128.0f / 2.0f) - 240.0f;
				TskBuf[id].vy = -0.5f;
			}
			...
			setTSKoption(id, TskBuf[id].level, BULLET_ZAKO03);
			TskBuf[id].energy = 2 + cast(int)(getRank() * 3);
			TskBuf[id].wait = WAIT_CNT;
			TskBuf[id].lock_mode = 0;
			TskBuf[id].lock_cnt = 30 - cast(int)(getRank() * 20);
			TskBuf[id].lock_id = -1;
		...
		case	1:	// first-move
			...
			if(waitEnemyStep2(id, &TSKenemy03Int)){
