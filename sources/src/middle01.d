...
void TSKmiddle01(int id)
{
	...
	switch(TskBuf[id].step){
		case	0:	// init
			...
			TskBuf[id].type = (Rand() % 8) + BULLET_ZAKO01;
			...
			if(max == 0) max = 1;
			for(int i = 0; i < max; i++){
				eid = setTSK(GROUP_03, &TSKmiddle01Option);
		...
		case	1:	// first-move
			...
			if(waitEnemyStep2(id, &TSKmiddle01Int)) TskBuf[id].step++;
			seqMove01(id);
		...
		case	100:
			...
			effSetBrokenBody(id, TskBuf[id].body_org, 16, 4,+0.0f,+0.0f,+3.0f,+3.0f);
			effSetBrokenBody(id, TskBuf[id].body_org, 20, 4,+0.0f,+0.0f,+3.0f,+3.0f);
			...
			effSetBrokenLine(id, TskBuf[id].body_org, 16, 4,+0.0f,+0.0f,+3.0f,+3.0f);
			effSetBrokenLine(id, TskBuf[id].body_org, 20, 4,+0.0f,+0.0f,+3.0f,+3.0f);
			TskBuf[id].step = -1;
			break;
...
void TSKmiddle01Draw(int id)
{
	...
	glEnd();
	glBegin(GL_POLYGON);
	for(int i = 16; i < 20; i++) setVertex(id, i);
	glEnd();
	glBegin(GL_POLYGON);
	for(int i = 20; i < 24; i++) setVertex(id, i);
	glEnd();
...
void TSKmiddle01Option(int id)
{
	...
	switch(TskBuf[id].step){
		case	0:	// init
			TskBuf[id].tskid |= TSKID_ZAKO;
			TskBuf[id].fp_exit = &TSKmiddle01OptionExit;
			TskBuf[id].tid = ship_id;
...
void TSKmiddle01OptionExit(int id)
{
	BulletCommand cmd = TskBuf[id].bullet_command;
