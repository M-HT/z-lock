	...
	switch(TskBuf[id].step){
		case	0:	// init
			...
			if(TskBuf[id].px < 0.0f){
				TskBuf[id].tx = +(cast(float)((Rand() % 128)));
			}else{
				TskBuf[id].tx = -(cast(float)((Rand() % 128)));
			}
			TskBuf[id].ty = (Rand() % 80) - (80.0f / 2.0f) - 240.0f;
			float[XY] tpos;
			...
			setTSKoption(id, TskBuf[id].level, BULLET_ZAKO02);
		...
		case	1:	// first-move
			...
			if(waitEnemyStep2(id, &TSKenemy02Int)){
		...
		case	100:
			...
			effSetBrokenBody(id, TskBuf[id].body_org,  0, 3,+0.0f,+0.0f,+3.0f,+3.0f);
			effSetBrokenBody(id, TskBuf[id].body_org,  2, 3,+0.0f,+0.0f,+3.0f,+3.0f);
			effSetBrokenLine(id, TskBuf[id].body_org,  0, 3,+0.0f,+0.0f,+3.0f,+3.0f);
			effSetBrokenLine(id, TskBuf[id].body_org,  2, 3,+0.0f,+0.0f,+3.0f,+3.0f);
			TskBuf[id].step = -1;
			break;
...
void TSKenemy02Draw(int id)
{
	...
	glBegin(GL_POLYGON);
	for(int i = 0; i < 3; i++) setVertex(id, i);
	glEnd();
	glBegin(GL_POLYGON);
	for(int i = 2; i < 5; i++) setVertex(id, i);
	glEnd();
